...
for tool in losetup blkid btrfs rsync curl depmod sed awk tar zstd pacman; do
...
# Resume: if a previous run already built everything in the overlay, skip
# the download/compile and go straight to payload extraction.
if compgen -G "$UPPER/usr/lib/modules/$KVER/updates/dkms/nvidia.ko*" >/dev/null; then
  log "Overlay already contains a built nvidia module — reusing previous build"
else
  ...
  log "Installing headers + dkms + nvidia (compiles the module, takes a few minutes)"
  in_chroot "pacman --config $PACCONF -U $PACOPTS /tmp/headers.pkg.tar.zst"
  in_chroot "pacman --config $PACCONF -S $PACOPTS dkms nvidia-open-dkms nvidia-utils lib32-nvidia-utils"
...
in_chroot "pacman -S --noconfirm --needed dkms nvidia-open-dkms nvidia-utils lib32-nvidia-utils"
compgen -G "$MERGED/usr/lib/modules/$KVER/updates/dkms/nvidia.ko*" >/dev/null \
  || in_chroot "dkms autoinstall -k $KVER"
compgen -G "$MERGED/usr/lib/modules/$KVER/updates/dkms/nvidia.ko*" >/dev/null \
...
# propagate the self-healing machinery so the NEXT update is covered too
mkdir -p "$NEWROOT/usr/lib/steamos-nvidia"
cp -a /usr/lib/steamos-nvidia/repatch.sh "$NEWROOT/usr/lib/steamos-nvidia/repatch.sh"
...
$( case $UPDATE_MODE in
     selfheal) echo "  Updates: SELF-HEALING — updating from within Steam works; the driver is
           rebuilt for each new OS version automatically (adds 10-20 min per
           update; failed rebuilds cancel the update, system stays working)." ;;
